...
/// converts status from char* format to bitmap representation
/// @param status char* string with ups status (for example "OL CHRG")
/// @return uint16_t status bitmap
uint16_t upsstatus_to_int(const char* status);

/// converts status from char* format to bitmap representation
/// @param status std::string string with ups status (for example "OL CHRG")
/// @return uint16_t status bitmap
uint16_t upsstatus_to_int(const std::string& status);

/// converts status from uint16_t bitmap to text representation
/// @param status uint16_t bitmap representing UPS status (for example STATUS_CHRG|STATUS_OL)
/// @return std::string text representation (for example "OL CHRG")
std::string upsstatus_to_string(uint16_t status);

/// converts status from std::string bitmap to text representation
/// @param status std::string bitmap representing UPS status (for example "12")
/// @return std::string text representation (for example "OL CHRG")
///
